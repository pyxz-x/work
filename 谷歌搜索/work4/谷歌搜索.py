    ...
    if len(result_div) != 0:
        for div in result_div:
            ...
            if (
                ...
                open("result2.json", "a", encoding="utf").write(
                    json.dumps(all_json, ensure_ascii=False) + "\n"
                )
                print(input__, "\t", page, "\t", all_json)
...
if __name__ == "__main__":
    ...
    data_jsons = []
    with open(
            "zuzhuang1_pdf.json", "r", encoding="utf-8"
    ) as f:
        datas = f.readlines()
        for data in datas:
    ...
    for data_json in data_jsons:
        try:
            ...
            input__ = data_json["input"]
            if 'source' in data_json.keys():
                source = data_json["source"]
            else:
                source = ''
            language = data_json["lang"]
            page = 0
            if finished_json and input__ in list(finished_json.keys()):
                input_, _ = list(finished_json.items())[-1]
                if input__ == input_:
                    print(f"{input__}\t上次最后，可运行")
                    input_ = _[0]
                    language = _[1]
                    page = _[2] + 1
                else:
                    print(f"{input__}\t已获取，跳过")
                    continue
            cookies = get_cookies()
            while len(cookies) == 0:
                sleep(15)
                cookies = get_cookies()
            # cookies['GOOGLE_ABUSE_EXEMPTION'] = "ID=09199fe48ec65498:TM=1760167877:C=r:IP=154.92.130.86-:S=gPNqYQjqQzrJqp_MV0rVYMA"
            print(cookies)
            print(f'获取{input__}\t{page}')

            result = to_json(cookies, input__, source, language, page)

            while result:
                finished_json_[input__] = [input__, language, page]
                open("finished.json", "w", encoding="utf-8").write(
